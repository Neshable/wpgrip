<div>
    @php
        $all        = collect($plugins);
        $withUpdate = $all->filter(fn($p) => !empty($p['update_version']));
        $active     = $all->filter(fn($p) => $p['status'] === 'active' && empty($p['update_version']));
        $inactive   = $all->filter(fn($p) => $p['status'] === 'inactive');
        $totalCount = $all->count();
        $updateCount = $withUpdate->count();
        $activeCount = $all->filter(fn($p) => $p['status'] === 'active')->count();
        $inactiveCount = $inactive->count();
    @endphp

    {{-- ── HEADER SECTION ─────────────────────────────────────────────── --}}
    <x-filament::section>
        <x-slot name="heading">Plugins</x-slot>

        <x-slot name="headerEnd">
            <button
                wire:click="syncPlugins"
                ...
                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium
                       ...
                       disabled:opacity-60"
            >
                <span wire:loading.remove wire:target="syncPlugins">
                    <x-filament::icon icon="heroicon-m-arrow-path" class="w-4 h-4" />
                </span>
                <span wire:loading wire:target="syncPlugins">
        ...
        <div class="flex flex-wrap gap-3">
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-300">
                <x-filament::icon icon="heroicon-m-squares-2x2" class="w-4 h-4 text-gray-400" />
                {{ $totalCount }} total
            </div>
    ...
    >
        ...
        <x-slot name="description">{{ $updateCount }} plugin{{ $updateCount > 1 ? 's have' : ' has' }} a newer version ready to install.</x-slot>

        <div class="overflow-hidden rounded-lg border border-amber-200 dark:border-amber-800/50">
            @foreach ($withUpdate as $i => $plugin)
                @include('livewire.plugins-panel-row', ['plugin' => $plugin, 'highlight' => 'update', 'last' => $loop->last])
            @endforeach
        </div>
    </x-filament::section>
    @endif
    ...
    >
        ...
        <x-slot name="description">{{ $activeCount }} plugin{{ $activeCount !== 1 ? 's' : '' }} currently running on your site.</x-slot>

        @if ($active->isNotEmpty())
        <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
            @foreach ($active as $plugin)
                @include('livewire.plugins-panel-row', ['plugin' => $plugin, 'highlight' => 'active', 'last' => $loop->last])
            @endforeach
        </div>
        @else
        <p class="text-sm text-gray-400 dark:text-gray-500 italic">All active plugins have updates pending.</p>
        @endif
    </x-filament::section>
    ...
    >
        ...
        <x-slot name="description">{{ $inactiveCount }} plugin{{ $inactiveCount !== 1 ? 's are' : ' is' }} installed but not active.</x-slot>

        <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 opacity-75">
            @foreach ($inactive as $plugin)
                @include('livewire.plugins-panel-row', ['plugin' => $plugin, 'highlight' => 'inactive', 'last' => $loop->last])
            @endforeach
        </div>
    </x-filament::section>
    @endif
    ...
    <x-filament::section class="mt-4">
        <div class="flex flex-col items-center py-10 text-gray-400 dark:text-gray-500">
            <x-filament::icon icon="heroicon-o-puzzle-piece" class="w-12 h-12 mb-3 opacity-40" />
            <p class="text-base font-medium">No plugins found</p>
            <p class="text-sm mt-1">Click <strong>Sync list</strong> to pull plugins from your site.</p>
        </div>
    </x-filament::section>
    @endif
</div>
