...
class SshAndChangeBranch implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct( Repository $repository, Site $site, string $branch )
    {
    ...
    /**
     * This is the main command for changing the branch
     *
     * @param [type] $connection
     * @return void
     */
    public function executeGitCommand($connection)
    {
        ...
        'git pull origin ' . $this->branch;  // Pull latest changes
     ...
     * Updates the pivot table with status, branch (if successful), status_text, and last_pull.
     *
     * @return void
     */
    public function saveToDb()
    {
        if( $this->pivot ) {
            ...
            // Only update the branch if the operation was successful
            if( $this->status === RepoStatus::SUCCESS->value ) {
                $updateData['branch'] = $this->branch;
            }
            
            $this->repository->sites()->updateExistingPivot($this->site->id, $updateData);
        }
    }
